...
from app.ai.openrouter_client import classify_case, OPENROUTER_API_KEY
...
def run_openrouter_checklist():
    print("=== STARTING OPENROUTER CLASSIFICATION CHECKLIST VERIFICATION ===")
    ...
    api_key_str = str(OPENROUTER_API_KEY) if OPENROUTER_API_KEY else ""

    if api_key_str and len(api_key_str) > 5 and not api_key_str.startswith("your_openrouter"):
    ...
    print(f"\n=== ALL 4 OPENROUTER CHECKLIST ITEMS PASSED SUCCESSFULLY ===")
    print(f"Single Call Baseline Latency: {latency_ms:.2f} ms")

if __name__ == "__main__":
    run_openrouter_checklist()
